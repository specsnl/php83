...
The three Dockerfiles use different base images but all track the same PHP 8.3.x patch series:
...
1. Check Docker Hub for the latest `php:8.3.x-fpm-trixie` tag. Only consider patch updates within the
`8.3.x` series — do not update to a different minor or major PHP version.
2. Check Docker Hub for the latest `dunglas/frankenphp` tag that matches the pattern
`x.x.x-php8.3.x-trixie`. Only consider updates that keep the PHP 8.3.x series.
